...
let {TMP_STATIC_FILES_DIR, STATIC_PREFIX, memfs} = require('./serve-file-middleware.js');
...
function IFramePreprocessor(logger) {
	...
	return function handleFile(content, file, done) {
		...
		let template = TEMPLATE
			...
			.replace('%SCRIPTS%', `
%SCRIPTS%
<script type="text/javascript" src="${STATIC_PREFIX}${transformedFilePath}?${+file.mtime}"></script>
`);
		
		// Save the file contents to the temp dir for serving it later
		memfs.writeFile(`${TMP_STATIC_FILES_DIR}/${transformedFilePath}`, content, (err) => {
			if(err) {
