...
function createStaticFileMiddleware(logger, injector) {
	...
	var serveFile = common.createServeFile(memfs, null, config)
	...
	return function staticFileMiddleware(req, res, next) {
		...
		memfs.stat(file, (err, stats) => {
			if (err) {
				log.error(`memfs.stat(${file}) errored with`, error);
			}
			if (err || !stats.isFile()) {
...
module.exports = { TMP_STATIC_FILES_DIR, STATIC_PREFIX, createStaticFileMiddleware, memfs };